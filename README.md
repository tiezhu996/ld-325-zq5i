...
- **锁价单闭环**：在 2–4 款对比材料中各选一家「有货且达到起订量」的报价提交锁价；任一报价无效（不存在、缺货/非在售、未达起订量）整单拒绝且不落任何记录（422）。每款材料同一时刻只保留一张有效锁价单，重复提交返回 409 冲突。锁价单保存的是**提交价快照**，供应商之后改价不会回写；供应商一旦把报价置为缺货或非在售，关联锁价单在同一事务内立即失效，刷新后只能回读为「已失效」。页面展示单号、快照价与失效状态。
...
| GET/POST | `/api/v1/lock-orders` | 查询锁价单（含失效单与快照价）/ 提交锁价单；任一报价无效整单拒绝 422、每款重复锁价返回 409 |
